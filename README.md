...
> 현재: **Phase 1 (MVP) + Phase 2 일부 — 파일럿**. 가입 없이 브라우저에서 바로 동작합니다.
...
- **Excel 내보내기** — SheetJS, 클라이언트 사이드 (단일 / 유형별 일괄)
...
```bash
# 방법 1) 정적 서버 — 작성/룰 검토/Excel/저장 전부 동작 (LLM 검토는 fallback)
cd qadoc
...
> Excel 내보내기(SheetJS)는 CDN에서 로드하므로 최초 실행 시 인터넷이 필요합니다.
...
src/
  ...
  js/
    ...
    export-excel.js     SheetJS Excel 내보내기
    app.js              상태/렌더링/이벤트
```
...
- **Phase 3** — PPT 내보내기(PptxGenJS), 공유 링크, 버전 이력 UI
